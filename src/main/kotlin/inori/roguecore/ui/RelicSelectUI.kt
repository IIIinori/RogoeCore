...
object RelicSelectUI {
    ...
    private fun describeEffectType(type: RelicEffectType): String {
        return when (type) {
            ...
            RelicEffectType.SHOP_FIRST_DISCOUNT -> "首购折扣"
            RelicEffectType.GAMBLE_FAIL_PROTECTION -> "败局保护"
            RelicEffectType.SEALED_CHEST_BONUS -> "封印宝箱"
            RelicEffectType.SHRINE_PURIFY_REWARD -> "净化奖励"
            RelicEffectType.TRIAL_REFUND_MATERIAL -> "试炼返材"
            RelicEffectType.ROUTE_STREAK_REWARD -> "路线奖励"
            RelicEffectType.FORGE_ACCELERATION_DISCOUNT -> "打造加速"
            RelicEffectType.IDENTIFY_ACCELERATION_DISCOUNT -> "鉴定加速"
            RelicEffectType.ROOM_CLEAR_SHARD_BONUS -> "清房收益"
            RelicEffectType.ELITE_SHARD_BONUS -> "精英赏金"
            RelicEffectType.BOSS_SHARD_BONUS -> "Boss 赏金"
            RelicEffectType.HIDDEN_SHARD_BONUS -> "隐藏收益"
            RelicEffectType.CHEST_SHARD_BONUS -> "宝箱碎片"
            RelicEffectType.LOW_HEALTH_CASHOUT_BONUS -> "险境撤离"
            RelicEffectType.MATERIAL_WORKSHOP_DISCOUNT -> "工坊折扣"
            RelicEffectType.SALVAGE_MATERIAL_BONUS -> "分解返材"
            RelicEffectType.RELIC_CHARGE_ON_ROOM -> "清房充能"
            RelicEffectType.RELIC_CHARGE_ON_EVENT -> "事件充能"
            RelicEffectType.RELIC_SPEND_CHARGE_FOR_REWARD -> "充能兑现"
            RelicEffectType.RELIC_CONSUME_ON_TRIGGER -> "触发消耗"
            RelicEffectType.RELIC_PAY_DEBT_WITH_CHARGE -> "债务担保"
        }
    }
}
